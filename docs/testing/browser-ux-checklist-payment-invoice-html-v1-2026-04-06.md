...
**Latest MCP re-check:** 2026-04-06 (agent, **fifth pass**) — **`http://127.0.0.1:3005`** + API **`http://127.0.0.1:3001`**. After **`prisma migrate deploy`** (fixes **`P2022`** on **`payments.invoice_number`**). Invoice HTML (structured v2): **`<main id="invoice-content">`**, **H1** + **INV-*** sequential number + issue date, line-items / totals / payment-status tables, **toolbar** (Print/Save PDF, Copy link, Share, Download HTML), **`Cache-Control: no-store`**. **View invoice** link on deal detail: **`aria-label`** with amount, opens API URL (new tab not exercised in this pass; same as prior runs).  
