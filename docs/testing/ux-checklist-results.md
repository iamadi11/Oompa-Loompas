...
| **Sign in** via MCP click + fill | **Fail (tooling)** | **Retry:** `curl -X POST http://127.0.0.1:3000/api/v1/auth/login` → **Pass** (200) with same-origin rewrites + valid user. MCP network log still shows **no** `fetch` to login — no `Signing in…` / no POST (React client handlers not observed for Sign-in in Cursor Browser). Use **real Chrome** or **Playwright** for E2E. **Do not use `browser_lock`** before MCP. |
...
| **Login GET leaked credentials**     | `type="submit"` + default `method="get"` leaked query params; **`type="button"`** + **`method="post"`** + **`readNamedInput`**. See [`LoginForm.tsx`](../../apps/web/components/auth/LoginForm.tsx).                                                                                                                                                         |
