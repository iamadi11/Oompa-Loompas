...
import { attentionQueueExportFilename } from '@oompa/utils'
import { api } from '@/lib/api'

/**
 * Download the full attention queue as CSV (same ordering as GET /attention, export capped server-side).
 */
export function ExportAttentionCsvButton() {
  ...
  async function handleExport() {
    ...
    try {
      ...
      a.href = url
      a.download = attentionQueueExportFilename(new Date())
      a.rel = 'noopener'
  ...
  return (
    <div className="flex flex-col items-start gap-1 sm:items-end">
      ...
      >
        ...
        {loading ? 'Exporting…' : 'Export queue CSV'}
      </button>
      {error && (
        <p className="text-xs text-red-600 text-right max-w-xs" role="status">
          {error}
        </p>
      )}
    </div>
