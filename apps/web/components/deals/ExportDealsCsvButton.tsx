...
import { dealsPortfolioExportFilename } from '@oompa/utils'

/**
 * ExportDealsCsvButton — download authenticated portfolio CSV (deal-level rows).
 *
 * Inputs: none (uses session cookie via same-origin fetch)
 * Outputs: triggers browser download of UTF-8 CSV
 * Failure modes: API error → inline message; user can retry
 */
export function ExportDealsCsvButton() {
  ...
  async function handleExport() {
    ...
    try {
      ...
      a.href = url
      a.download = dealsPortfolioExportFilename(new Date())
      a.rel = 'noopener'
